...
> **权威设计**：[`design.md`](design.md)（v0.1.13, 2026-07-30，含工业级审核 patch F-1..F-19 + Changelog §13.1..§13.15 + §13.PF pre-flight retractions + Phase 3C counter wiring §14 + Phase 3D batch tile-fill §15 + Phase 3E world↔cell math §16 + Phase 3F sprite culling §17 + Phase 3G chunk-source budget §18）。代码与 design.md 不一致时，design.md 优先。

## 当前状态 — P3G.2a in-AY2D CPU soft cap (2026-07-30)

- `design.md` 是权威设计（v0.1.13 + audit F-1..F-19 + Changelog §13.1..§13.15 + §13.PF + §14 P3C + §15 P3D + §16 P3E + §17 P3F + §18 P3G）。代码与 design.md 不一致时，design.md 优先。
- 子模块 HEAD = P3G.2a：`TilemapBudget` 加 `maxChunksCpuSoftCap`（3-tier residency model：hard cap → soft cap → GPU residency）；`Ay2DCounters` 加 `chunk_io_residency_reject` 第 11 字段（cumulative, resetAll only）；`InMemoryTilemapChunkSource` 加 `setMaxChunksCpuSoftCap(uint32_t)` + private `evictDownTo(uint32_t)` helper；`setBudget(b)` 走 soft cap 路径。
- `unittest/` 现在有 **20** 个 test 文件 / **600** CHECK assertions PASS（P3H.2 19/559 → P3G.2a 20/600 = +1 suite, +41 CHECK）。新增 `Test_CpuSoftCap` 走真 wired delta（4 case：setSoftCap trim / softCap >= hardCap no-op / setBudget applies both / residency_reject reset discipline）。
- 锁行为（§18.6 / R-3G.1..7 + §13.PF + §13.15）：**R-3G.1** non-LRU policy = no-op + false（active lock；Distance/TimeWindow 留给 Phase 4 streaming 跨模块 PR，§13.PF clarification）；**R-3G.2** 累计 counter 纪律扩展到 `chunk_io_residency_reject`（resetAll only，resetPerFrame 不 zero）；**R-3G.3a** `maxIoBytesPerSec == 0` 关闭 rate gate；**R-3G.4** `maxChunksResident` 是 GPU residency 撞 §4.2.1（cross-module PR to RenderResourceManager，Phase 6 仍 defer）；§13.15 P3G.2a soft cap 语义 = "non-zero AND less than hard cap → evict-down-to；non-zero AND >= hard cap → no-op；0 → disabled"。
- 不破坏 P3A / P3B / P3C / P3D / P3E / P3F / P3G / Phase 5 / P3H.2 任何已有测试（counter append-only，soft cap 字段纯添加）。
...
- `cmake/CheckNoBgfxInPublicHeaders.cmake` 是 bgfx-leak guard (§11.2 / F-5)；双向验证已通过（20 public headers scanned, 0 leaks；P3G.2a 不加新 header，Ay2DCounters + TilemapBudget + InMemoryTilemapChunkSource 都是 bgfx-clean 已 ship headers）。
- 跨模块 PR 仍按 `design.md` §4.2.1 deferred：AYRenderer / AYResource / AYEntity / AYShader / AYPhysics maintainer 拥有各自的 merge gate。P3G.2a 不开任何跨模块 PR（CPU-only 切片）。
