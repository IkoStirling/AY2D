...
## 当前状态 — P3J.9 in-AY2D `requestChunkRow` bulk+dedup (A-6, 2026-07-31, **Phase 3J ship**)

- `design.md` 是权威设计（v0.1.30 + audit F-1..F-19 + Changelog §13.1..§13.33 + §13.PF + §14 P3C + §15 P3D + §16 P3E + §17 P3F + §18 P3G + §18.7 P3I.2 + §13.22 P3I.3 + §13.23 P3I.4 + §13.24 P3J.1 + §13.25 P3J-env + §13.26 P3J.2 + §13.27 P3J.3 + §13.28 P3J.4 + §13.29 P3J.5 + §13.30 P3J.6 + §13.31 P3J.7 + §13.32 P3J.8 + §13.33 P3J.9）。代码与 design.md 不一致时，design.md 优先。
- 子模块 HEAD = P3J.9：`InMemoryTilemapChunkSource::requestChunkRow(y, xStart, xEndExclusive)` 新 public method（half-open `[xStart, xEndExclusive)` + per-coord dedup — 已在 `_pending` 的 coord 复用现有 handle，零 rate-gate charge；未在的 delegate 到 `requestChunk` 单源）。**Phase 3J 唯一 surface-changing slice**。
- `unittest/` 现在有 **34** 个 test 文件 / **1372** CHECK assertions PASS（P3I 28/990 → P3J 34/1372 = +6 suite, +382 CHECK；9 个 P3J slice + 6 个新 suite Test_ChunkSourceEraseByKey / Test_SpriteDrawCmdLayout / Test_AtlasDescValidator / Test_EvictionPolicyEnum / Test_TileLoadStateEnum / Test_ChunkSourceRowCoalesce；3× consecutive **incremental** green locked + cold-configure green after J-env root PR）。bgfx-leak guard green。
- 锁行为（L-3J-1..L-3J-10 全部生效；L-3I-1..L-3I-13 持续生效；§3.4 epoch / §13.14 view shape / §13.19 view shape / §18.7 one-source-per-tilemap / L-3 / L-4 / R-10 全部守住）。
...
- `cmake/CheckNoBgfxInPublicHeaders.cmake` 是 bgfx-leak guard (§11.2 / F-5)；双向验证已通过（21 public headers scanned, 0 leaks；P3J.9 公共头增量仅 `InMemoryTilemapChunkSource::requestChunkRow` 一个方法声明 + 14 行 doc block，无 bgfx 路径）。
- **Phase 3J 完整 ship** (9/9 slice + 1 root ops: P3J.1 KI-3I-1 closure / P3J.2 SpriteDrawCmd layout 88→112B MSVC FVector2 SIMD align fix / P3J.3 AtlasDesc validator 全覆盖 / P3J.4 TileAnimation 100-entry batch / P3J.5 EvictionPolicy enum / P3J.6 TileLoadState enum / P3J.7 ChunkRequestHandle wrap-around / P3J.8 Sprite batch-state / P3J.9 requestChunkRow 唯一 feat; J-env root commit `43089c6` 修了 Phase 3I 踩坑 #41 cold-configure VCPKG_INSTALLED_DIR gap + 加 root `do_cmake.bat` 三层 dirname derive 闭环)。跨模块 PR (CM-1..CM-5) 仍按 `design.md` §4.2.1 deferred。**Phase 3K / 下一波 in-AY2D residue = 无**（Phase 3I + 3J 已清空所有 listed residue）。
